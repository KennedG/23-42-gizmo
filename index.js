...
//Entrada Maridões🚩
client.on('guildMemberAdd', async member => {
	let guild = await client.guilds.cache.get('917960090349350982');
	let channel = await client.channels.cache.get('917963044749979659');
	let emoji = await member.guild.emojis.cache.find(
		emoji => emoji.name === ':emoji_59:'
	);
	if (guild != member.guild) {
		...
		let embed = await new Discord.MessageEmbed()
			.setColor('#7c2ae8')
			.setAuthor(member.user.tag, member.user.displayAvatarURL())
			.setTitle(`<:x_BandeiraIDK:921834981767143495>𝐁𝐞𝐦 𝐯𝐢𝐧𝐝𝐨(𝐚)𝐬 𝐚𝐨𝐬 𝐌𝐚𝐫𝐢𝐝𝐨̃𝐞𝐬<:x_BandeiraIDK:921834981767143495> `)
			.setImage('https://i.imgur.com/IkgocvY.gif')
			.setDescription(`** <:branco_pessoaRDM:908562579117453342>${member.user}**, 𝐛𝐞𝐦-𝐯𝐢𝐧𝐝𝐨(𝐚) 𝐚𝐨 𝐌𝐚𝐫𝐢𝐝𝐨̃𝐞𝐬<:x_BandeiraIDK:921834981767143495>\n𝐀𝐭𝐮𝐚𝐥𝐦𝐞𝐧𝐭𝐞 𝐞𝐬𝐭𝐚𝐦𝐨𝐬 𝐜𝐨𝐦 **${member.guild.memberCount} 𝐦𝐞𝐦𝐛𝐫𝐨𝐬**, 𝐄𝐬𝐩𝐞𝐫𝐨 𝐪𝐮𝐞 𝐧𝐚̃𝐨 𝐟𝐚𝐜̧𝐚 𝐦𝐞𝐫𝐝𝐚 𝐚𝐪𝐮𝐢! \n <a:ccclonadl:921869007026536488> Depois que você entrou não pode sair! CADELA.<a:ccclonadl:921869007026536488> `)
			.setThumbnail(member.user.displayAvatarURL({
					dynamic: true,
					format: 'png',
					size: 1024
